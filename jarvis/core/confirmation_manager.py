...
class ConfirmationManager:
    ...
    # ------------------------------------------------------------------
    # Elicited-prompt decisions (#210)
    # ------------------------------------------------------------------
    #
    # A server can ask a question mid-tool-call (MCP elicitation). When the
    # elicitation policy (jarvis.core.elicitation) routes that question to a
    # HUMAN, the prompt is a plain yes/no ("did the human approve this?") — the
    # same shape a tool confirmation resolves to. These two helpers let the
    # elicitation runtime reuse this manager's notification channels without
    # touching the task-confirmation _pending machinery: has_live_channel says
    # whether any human is reachable at all (so a prompt with no surface can be
    # DECLINED rather than left hanging), and request_prompt_decision fires the
    # question on the best channel. The answer comes back as an ordinary
    # confirmation_response the elicitation runtime intercepts by request id.

    def has_live_channel(self, notification_silent: bool = False) -> bool:
        """True if any human confirmation channel is currently reachable.

        Mirrors the availability checks in ``_send_notification`` (TUI, desktop,
        socket, TTY). Used so an elicited prompt with no surface to answer it is
        declined immediately instead of waiting out a timeout — a credential
        prompt with nobody to answer must never be accepted with a guess.
        """
    ...
    async def request_prompt_decision(
        ...
        """Ask a human a yes/no question over the best notification channel.

        Reuses the same channel dispatch as a tool confirmation, but for a
        pre-attributed elicited prompt (``prompt_text`` already reads "the
        server X is asking …"). Deliberately does NOT register the request in
        ``_pending``: an elicited prompt is not a task batch and must not flow
        into ``resolve``'s task logic. The channels inject an ordinary
        ``confirmation_response`` keyed by ``request_id``; the elicitation
        runtime intercepts it before the normal confirmation path runs.

        Non-blocking, like ``request_confirmation`` — the answer arrives later
        as an event. ``timeout`` is passed to the channels only; the caller
        owns the authoritative decline-on-no-answer timer.
        """
        tool_lines = [prompt_text]
        tool_names = [server or "server"]
        tools_detail = [{"tool_name": prompt_text, "task": {}, "params": {}}]
        logger.info(
            "Elicited-prompt decision requested (non-blocking): id=%s", request_id
        )
        await self._send_notification(
            request_id=request_id,
            tool_names=tool_names,
